<div class="tcard">
  <a href="{{ route('trip.show', $trip) }}" class="tcard-body">
    <div class="tcard-head">
      <span class="tag tag-primary"><x-icon name="payments" :size="14" /> {{ strtoupper($trip->currency) }} {{ number_format($trip->budget_total) }}</span>
  ...
  <div class="tcard-foot">
    ...
    @endif
    <span class="tstats"><x-icon name="favorite" :size="16" /> {{ $trip->likes_count ?? $trip->likes()->count() }} · <x-icon name="chat_bubble" :size="16" /> {{ $trip->comments_count ?? $trip->comments()->count() }}</span>
  </div>
</div>
